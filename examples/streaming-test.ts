...
async function main() {
  ...
  await streamChars(markdown);

  // 2. Inline prefix preservation — short line (should NOT erase prefix)
  await streamChars("Here is some **bold** and *italic* text.", "AI: ");
  ...
  await streamChars("What is `yoctomarkdown`?", "Q: ");
  await streamChars(
    "A **tiny** streaming markdown highlighter for the terminal.",
    "A: ",
  );

  // 5. Unicode width edge cases
  // CJK wide characters (each takes 2 cells)
  await streamChars("日本語のテスト — **太字** and `コード` inline.", "CJK: ");
